...
def main():
	orb = cv2.ORB_create(1000)  # ORB keypoints detector
	...
	bbox = np.float32([
		[row,col],[row,col+w],[row+h,col+w],[row+h,col] # Create 4x2 matrix represents the pixel coordinate of 4 corners of the bbox defined by row, col, h, w. Each corners occupy a row.
	...
	while cap.isOpened():
		...
		gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) #Added
		...
		entire_mask = np.ones(gray_frame.shape)
		keypts, desc = orb.detectAndCompute(gray_frame,entire_mask.astype(np.uint8)) # On entire frame
